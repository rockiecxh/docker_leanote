...
*  默认网址路径 http://访问网址或IP:8080

## 说明Description
* 自动备份及导入还原说明
  * 备份路径设置在`[宿主机储存路径]/leanotedata/backup`下
  * 自动备份时间为每天0点，备份默认保存3天，可自行调整环境变量DAYS修改（修改为0关闭自动备份），过期自动删除
  * 一份备份共有两个文件
    * mongodb_bak为数据库备份
    * Leanote_bak为附件及配置文件备份
  * 还原/数据导入/迁移方法：
...
* 参数说明
  * `-m 512M`为限制内存，防止docker占用过多内存影响宿主机，可以自由调整，空载环境约占用100M内存
  * `--cpus=1`为限制使用单核CPU，默认情况一般cpu使用率低于10%，备份导出时单核100%持续不足30s左右
  * `-p 8080:9000`Leanote默认端口为9000，上述命令将端口映射到了8080端口，可以根据个人需要调整
  * **必填ENV**  SITEURL，填写为你的访问网址/IP+端口，需要注意的是，填写的是各种端口转发之后的网址+端口，比如用nginx转发到9999端口，即填写http://abc.com:9999 ，如按照上述提供的启动命令无其他端口转发请填写`http://url:8080`即可。**不正确的填写会导致客户端无法同步图片等附件**。
